...
Everything the film says is also in the served HTML — inside the contact
card, in the tree but off the screen, for screen readers and fetchers — and
at [`/llms.txt`](https://elliotgreenbaum.com/llms.txt), so an assistant
handed the address gets the story rather than a name and three links. Both are generated from `src/content/film.json` at build time by
...
(`api/chat.ts`) around `server/chat.ts`, which calls the Claude API with a
page of notes about me as its system prompt and streams the reply back. The
notes are not in this repository: `server/persona.ts` reads them from
`ELLIOT_PERSONA` in the environment, or from a gitignored `server/persona.md`
beside it on a local run.
...
is the notes above; what the panel says on its own — the greeting, the

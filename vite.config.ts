 ...
 *
 * About those headers (vercel.json cannot carry a comment: Vercel rejects any
 * key it does not know, and a `_` note there broke the deploy on 2026-09-15):
 * Security headers for every response. The CSP hash is the SHA-256 of the
 * inline <script> in index.html (the WebGL probe, which has to stay inline to
 * run before first paint): change one character of that script and the hash
 * has to change with it — tools/csp-hash.mjs prints the current one, and
 * verify goes red if they disagree. img-src allows Spotify's image host for
 * the album art in the panel. The two posthog.com hosts are analytics
 * (src/core/analytics.ts): events go to us.i.posthog.com, and the library
 * lazy-loads its recorder and web-vitals modules from us-assets.i.posthog.com;
 * without both, a key in Vercel would send nothing and fill the console with
 * CSP errors. The same headers are put on the preview server by
 * vite.config.ts, so the gate runs under them.
 */
